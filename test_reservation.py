...
def test_create_reservation_weekly_limit_400(client):
    ...
    assert client.post("/api/reservations/", json=payload(date_s=d0, start=9, end=10, participants=participants_ok("501000"))).status_code == 200
    assert client.post("/api/reservations/", json=payload(date_s=d0, start=10, end=11, participants=participants_ok("501001"))).status_code == 200

    assert client.post("/api/reservations/", json=payload(date_s=d1, start=9, end=10, participants=participants_ok("501002"))).status_code == 200
    assert client.post("/api/reservations/", json=payload(date_s=d1, start=10, end=11, participants=participants_ok("501003"))).status_code == 200

    assert client.post("/api/reservations/", json=payload(date_s=d2, start=9, end=10, participants=participants_ok("501004"))).status_code == 200

    r = client.post("/api/reservations/", json=payload(date_s=d3, start=9, end=10, participants=participants_ok("501005")))
    assert r.status_code == 400
